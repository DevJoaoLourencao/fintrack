...
function Tile({ label, value, valueClass, accentClass, hideValues }: TileProps) {
  return (
    <div className="relative overflow-hidden rounded-2xl border border-border/50 bg-card px-5 py-5 shadow-card-md">
  ...
  if (isLoading) {
    return (
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
        {[0, 1, 2].map((i) => (
          <div key={i} className="relative overflow-hidden rounded-2xl border border-border/50 bg-card px-5 py-5 shadow-card-md animate-pulse">
